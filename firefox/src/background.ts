// background.ts (Firefox MV2 WebExtension)
...
async function handleAlarms(alarm: { name: string }): Promise<void> {
...
async function handleInstallOrUpdate(details: { reason: string }): Promise<void> {
...
function handleMessage(request: { action: string }): void {
...
async function popupBg(manualCall = false): Promise<void> {
...
async function sendStopSearch(): Promise<void> {
    await browser.storage.sync.set({ isSearching: false });
...
async function checkLastOpened(): Promise<void> {
